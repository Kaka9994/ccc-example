...
export default class CustomAssembler extends cc.Assembler {
    ...
    protected _verticesCount: number = 4; // 顶点数量(一个四边形4个顶点)
    protected _indicesCount: number = 6;  // 顶点索引数量(一个四边形按照对角拆分成2个三角形，2*3 = 6个顶点索引)
    protected _colorOffset: number = 4;   // 颜色值在顶点结构里的偏移(下标从4开始算)
    protected _uvOffset: number = 2;      // uv值在顶点结构里的偏移(下标从2开始算)
    protected _floatsPerVert: number = 5; // 顶点结构占用float数量(vfmtPosUvColor 结构占5个float32)
    ...
    protected updateColor(comp, color: number): void {
